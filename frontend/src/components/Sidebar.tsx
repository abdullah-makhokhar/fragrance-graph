    ...
    <div className="w-80 h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-2xl z-20 overflow-hidden">
      ...
      <div className="p-6 pb-4">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-300 to-indigo-800 bg-clip-text text-transparent font-display tracking-tight">
              Fragrance Accord Graph
            </h1>
          ...
          <button
            ...
            className="w-20 h-10 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all text-lg"
          >
            {darkMode ? 'Light' : 'Dark'}
          </button>
        </div>
      ...
      <div className="px-6 flex gap-1 mb-4">
        {[
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 ${activeTab === tab.id
            ...
            {[
              ...
              <div key={section.id} className="border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden bg-white dark:bg-slate-900/50">
                <button
                  ...
                  className={`w-full px-4 py-3 flex justify-between items-center transition-all ${expandedSection === section.id ? 'bg-slate-50 dark:bg-slate-800' : ''
                    }`}
                >
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {(section.attr || []).slice(0, 100).map((val: any) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full text-[11px] font-semibold transition-all border ${(filters[section.id as keyof Filters] as Set<any>).has(val)
